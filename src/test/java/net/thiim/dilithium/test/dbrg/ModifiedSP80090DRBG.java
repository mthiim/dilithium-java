...
/**
 * Slightly modified P800-90A CTR DRBG for compatibility with the one used in KAT tests in the Dilithium reference implementation. Specifically, the following processing of the
 * seed material is skipped during initialization:
 * 
 *         //byte[] seed = Block_Cipher_df(seedMaterial, _seedLength);
 */
public class ModifiedSP80090DRBG
    implements SP80090DRBG
{
     ...
     */
    public ModifiedSP80090DRBG(BlockCipher engine, int keySizeInBits, int securityStrength, EntropySource entropySource, byte[] personalizationString, byte[] nonce)
    {
    ...
    * 6. Return output_block. 
    ...
    public int generate(byte[] output, byte[] additionalInput, boolean predictionResistant)
    {
        if (_isTDEA)
        {
            ...
            if (isTooLarge(output, TDEA_MAX_BITS_REQUEST / 8))
            {
        ...
        else
        {
            ...
            if (isTooLarge(output, AES_MAX_BITS_REQUEST / 8))
            {
    ...
    private boolean isTooLarge(byte[] output, int i) {
